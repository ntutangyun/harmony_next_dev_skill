...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/archive-file-compression-guidelines_
...
压缩解压缩接口的输入输出路径均为沙箱路径，具体请参考应用沙箱目录。
...
调用OH_Archive_Writer_OpenFile创建归档写入器。需指定输出文件路径、打开模式（如OH_ARCHIVE_OPEN_MODE_CREATE）和归档格式（如OH_ARCHIVE_FMT_ZIP）。

（可选）调用OH_Archive_Writer_SetCompressMethod设置压缩算法和压缩级别。默认使用DEFLATE压缩算法，压缩级别为6，开发者可根据需要调整。

（可选）调用OH_Archive_Writer_SetProgressHandlerWithData设置进度回调，用于获取压缩进度信息。

调用OH_Archive_Writer_Add向归档中添加文件或目录，支持同时添加多个文件或目录。

调用OH_Archive_Writer_Close完成归档写入并释放资源。压缩完成后必须调用此接口释放资源。
...
调用OH_Archive_Reader_OpenFile打开归档文件并创建读取器。需指定ZIP文件路径。

（可选）调用OH_Archive_Reader_SetProgressHandlerWithData设置解压缩进度回调。

调用OH_Archive_Reader_ExtractAllFile将归档文件解压缩到指定目录。解压缩后的文件将保持原有的目录结构。

调用OH_Archive_Reader_Close关闭归档读取器。解压缩完成后必须调用此接口释放资源。
