...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/archive-stream-compression-guidelines_
...
创建流式压缩配置并调用OH_Archive_StreamWrite_Create创建流式压缩写入器。需指定配置信息，包括block大小、线程数、压缩算法和校验方式。

调用OH_Archive_StreamWrite_Start启动流式压缩。压缩过程中通过输出回调函数处理压缩后的数据。

调用OH_Archive_StreamWrite_SetCompressLevel设置压缩级别（可选，默认为6）。

多次调用OH_Archive_StreamWrite_Update输入数据，适合流式处理。

调用OH_Archive_StreamWrite_End结束流式压缩并获取结果信息。

调用OH_Archive_StreamWrite_Destroy销毁流式压缩写入器，释放资源。
...
调用OH_Archive_StreamWrite_Create创建流式压缩写入器。

调用OH_Archive_StreamWrite_Start启动流式压缩。

多次调用OH_Archive_StreamWrite_Update输入压缩数据。

调用OH_Archive_StreamWrite_Cancel取消流式压缩。

调用OH_Archive_StreamWrite_End结束流式压缩。
...
创建流式解压缩配置并调用OH_Archive_StreamRead_Create创建流式解压缩读取器。需指定配置信息，需与压缩时的配置一致。

调用OH_Archive_StreamRead_Start启动流式解压缩。解压缩过程中通过输出回调函数处理解压缩后的原始数据。

多次调用OH_Archive_StreamRead_Update输入压缩数据进行解压缩。

调用OH_Archive_StreamRead_End结束流式解压缩并获取结果信息。

调用OH_Archive_StreamRead_Destroy销毁流式解压缩读取器，释放资源。
...
调用OH_Archive_StreamRead_Create创建流式解压缩读取器。

调用OH_Archive_StreamRead_Start启动流式解压缩。

多次调用OH_Archive_StreamRead_Update输入压缩数据。

调用OH_Archive_StreamRead_Cancel取消流式解压缩。

调用OH_Archive_StreamRead_End结束流式解压缩。
