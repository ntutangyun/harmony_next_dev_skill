...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/core-vision-introduction_
...
主体分割：可以检测出图片中区别于背景的前景物体或区域（即“显著主体”），并将其从背景中分离出来，适用于需要识别和提取图像主要信息的场景，广泛运用于前景目标检测和前景主体分离的场景。
...
图像超分：图像超分一项基于人工智能算法的图像处理功能，可将低分辨率、模糊或细节不足的图片智能放大并增强清晰度。在放大图像尺寸的同时，系统能够自动补全纹理细节、优化边缘轮廓、降低噪点干扰，使图片呈现出更清晰、更自然、更高质量的视觉效果。

通过文本搜索图片：基于自然语言理解与图像跨模态检索技术，支持用户输入任意描述性文本（如“海边的落日”、“证件”），从应用沙箱中精准匹配并提取符合描述的图片。该功能极大地提升了用户的搜图效率，让无标签、未分类的图像资产变得可检索、易触达，广泛应用于智慧相册、聊天对话（如即时通讯、社交互动）以及电商以文搜图等场景。
