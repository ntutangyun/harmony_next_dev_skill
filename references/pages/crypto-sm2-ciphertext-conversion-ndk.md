...
调用OH_CryptoSm2CiphertextSpec_Create，创建空的SM2密文规格对象。

调用OH_CryptoSm2CiphertextSpec_SetItem，设置密文的各个参数（C1.x、C1.y、C2、C3）。

调用OH_CryptoSm2CiphertextSpec_Encode，生成ASN.1格式的密文（当前密文转换仅支持SM3，实现中只校验hash长度是否为32字节）。

使用完毕后，调用OH_CryptoSm2CiphertextSpec_Destroy销毁SM2密文规格对象。
...
调用OH_CryptoSm2CiphertextSpec_Create，从ASN.1格式密文创建SM2密文规格对象。

调用OH_CryptoSm2CiphertextSpec_GetItem，获取密文中的各个参数（C1.x、C1.y、C2、C3）。
