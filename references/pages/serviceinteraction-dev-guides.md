...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/serviceinteraction-dev-guides_
...
导入serviceInteraction模块以及其他必要的工具模块。

import { serviceInteraction } from '@kit.ServiceCollaborationKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
import { util } from '@kit.ArkTS';

创建CollaborationConfig对象，配置触发类型、窗口ID等参数。
...
通过onServiceInteraction接口注册回调方法，处理设备连接、断连、消息接收等事件。
...
通过Connection对象的sendMessage方法向对端设备发送消息。
...
通过Connection对象的disconnect方法断开与对端设备的连接。
...
通过offServiceInteraction接口取消注册回调方法，停止接收回调事件。
...
准备两台设备，并确保WLAN和蓝牙开关已打开。

在两台设备上分别运行应用。

将两台设备顶端轻轻一碰，观察是否触发连接回调。

检查连接状态是否正确显示。

在一台设备上发送消息，观察另一台设备是否收到消息。

断开连接，观察断开回调是否正确触发。

检查hilog日志，确认所有操作是否正常执行。
