...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-analyze-config_

功能介绍

分析过程模板用于在RAG运行时，通过流式输出向用户实时呈现结构化的中间结果。该模板包含预设文本和动态占位符，可将大模型的输出或RAG执行中的关键信息组织为连贯文本。开发者可以通过在指定应用资源目录放置thought_template.json配置文件，自定义分析过程模板。
...
在应用资源目录的rawfile/arkdata/rag下创建thought_template.json文件，若工程中无目标目录则递归创建。
