...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/native-camera-preview-imagereceiver_
...
初始化图片接收器ImageReceiver实例，获取SurfaceId。
...
通过上一步获取到的SurfaceId创建预览流（在创建预览流之前需要将SurfaceId类型转成char *），参考预览(C/C++)步骤4。

创建会话，使能会话，参考会话管理(C/C++)。
