...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-watch_
...
这是符合预期的行为，展示了@Watch回调的触发时机是根据状态变量真正变化的时间。因为@Link直接同步，而@ObjectLink需要等父组件更新子组件变量。类似地，当父组件的数据源变化时，@Prop也会表现出与@ObjectLink类似的行为，其回调触发时间也会略晚。
