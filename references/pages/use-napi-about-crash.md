...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/use-napi-about-crash_
...
在crash文件的Maps中匹配十六进制地址，即可确定实际导致崩溃的函数具体是来自哪个so共享库。
...
napi_wrap、napi_wrap_enhance、napi_add_finalizer中的finalize_cb执行过程中发生崩溃。

napi_set_instance_data中finalize_cb执行过程中发生崩溃。

调用napi_async_work相关接口complete执行过程中发生崩溃。

调用threadsafe_function相关接口，thread_finalize_cb及call_js_cb执行过程中发生崩溃。
