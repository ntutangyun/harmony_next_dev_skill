...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/bm-tool_
...
表示当前系统账号的编号，系统账号的相关接口请参考@ohos.account.osAccount (系统账号管理)，下面给出几种常见的系统账号。
...
在启动调试或运行应用/服务时，由于安装应用的Profile签名文件中的类型与已安装应用的不相同，导致安装HAP出现错误。
...
如果存在，请查看对应的权限文档的要求，申请权限，同时请关注权限是否支持申请。例如：受限开放权限请参考文档申请受限权限。企业类应用可用权限仅企业类应用支持申请，三方应用不支持。
...
禁止通过bm命令安装Profile签名文件中的类型为app_gallery并且签名证书类型为release的应用。

可能原因

安装应用Profile签名文件中的类型为app_gallery并且签名证书类型为release。

处理步骤

使用Profile签名文件中的类型非app_gallery的文件对应用重新签名。
...
检查HAP包中的native软件包，替换正确的native软件包并重新签名打包。参考Native软件包开发指南。
...
检查是否存在进程占用相应的native软件包，若存在则结束进程后重新卸载。参考Native软件包开发指南。
...
应用配置的设备类型或者requiredDeviceFeatures标签配置的设备特性不支持安装。
...
虽然已卸载预置应用，但在安装新应用之前，系统仍会先安装预置应用包，随后再安装新应用包。这是因为预置应用的安装签名信息中的密钥和APP ID，与新安装的应用对应信息不一致。

处理步骤

方法一：重新签名。

通过重新签名，确保应用签名信息中的密钥和APP ID至少有一项与预置应用保持一致。

如果直接使用DevEco Studio自动签名，由于生成的签名信息均为随机值，与预置应用不匹配，将导致应用安装失败。因此，您需要用之前申请发布Profile的账号，重新申请调试Profile或直接使用发布Profile对应用重新签名，完成签名后，重新安装应用。
...
检查应用是否为预置的release版本。如果是，需要替换应用的Profile签名文件的类型，重新签名并安装。
...
该Profile签名文件中的类型被限制，禁止安装到当前设备中。
...
该应用的Profile签名文件为调试类型，且未配置当前设备的UDID。

处理步骤

方式一：根据指导将当前设备添加到调试类型证书或使用发布类型证书重新签名。
...
重新配置应用或者插件Profile签名文件中的 pluginDistributionIDs。配置格式如下：
...
参考权限申请指导申请ohos.permission.kernel.SUPPORT_PLUGIN权限。
...
应用Profile签名文件中allowed-acls字段的U1Enabled配置发生变更，例如：
...
方案一：重新签名，签名过程中，请参考自动签名的支持ACL权限、或者手动签名的使用ACL的签名配置指导进行配置，确保待安装应用与已安装应用配置一致。
...
多HAP包签名时使用的Profile签名文件不一致导致签名信息中allowed-acls的U1Enabled配置不一致。
...
在应用的module.json5配置文件中配置了executableBinaryPaths标签，但是应用未配置解压模式。

该设备不支持安装配置了executableBinaryPaths标签的应用。

处理步骤

配置应用为解压模式，即在应用的module.json5配置文件中设置compressNativeLibs标签为true。

更换为PC/2in1设备。

[h2]9568450 安装失败，应用包需为签名证书类型为debug的应用
...
开发者模式下使用-g参数授权签名证书类型为非debug的应用。可以通过Profile签名文件中的type字段来查看签名证书类型。
