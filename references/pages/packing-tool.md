...
检查module.json5，确保module.json5文件中正确配置了abilities标签，并且至少包含一个ability，详细请参见abilities标签。

检查module.json5和app.json5，当bundleType为atomicService时，确保installationFree配置为true，反之，需要配置为false。
...
检查module.json5，确保targetModuleName和requestPermissions不会同时出现。

检查module.json5，确保targetModuleName和name不同。

检查module.json5，确保配置targetPriority时，同时配置了targetModuleName。

检查app.json5和module.json5，确保配置targetBundleName时，同时配置了targetModuleName。
...
检查打包文件，删除HSP包中module.json5配置的dependencies。
