...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-key-derivation-using-hkdf_

对应算法规格请查看密钥派生算法规格：HKDF。

开发步骤

构造HKDFSpec对象，作为密钥派生参数进行密钥派生。
...
调用cryptoFramework.createKdf，指定字符串参数'HKDF|SHA256|EXTRACT_AND_EXPAND'，创建密钥派生算法为HKDF、HMAC函数摘要算法为SHA256、模式为提取和扩展的密钥派生函数对象（Kdf）。
