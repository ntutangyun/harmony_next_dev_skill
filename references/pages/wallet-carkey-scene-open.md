...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/wallet-carkey-scene-open_
...
DK服务器调用钱包服务器进行预置模板，将一些公共信息写入模板用于后面关联车钥匙实例。

收到车主App的申请请求之后，由DK服务器申请ICCE钥匙，然后将生成的车钥匙JWE数据返回车主App。

激活车钥匙过程中，DK服务器依次收到钱包App（经过钱包服务器代理）的设备认证、获取个人化数据token、获取个人化数据请求，返回结果给钱包App。

当车钥匙开通成功之后，DK服务器通过NFC相关事件回调通知接口接收到开通成功回调通知，请求体中包含账号+设备的唯一标识pushToken（此为原值，非sha256签名值）。DK服务器需要记录车钥匙唯一标识organizationPassId和账号+设备唯一标识pushToken的sha256签名值之间的关联关系，用于后续车主App端云对账展示车钥匙。

客户端开发

完整示例请参考客户端示例代码。

车主App调用canAddPass接口检查当前是否可以开通车钥匙，如果不满足开通条件，请按照返回结果及错误码进行引导处理。
...
车主App调用queryPassDeviceInfo接口，查询当前设备的设备类型、账号+设备标识等信息。
...
车主App携带设备信息请求DK服务器，由DK服务器申请ICCE钥匙，然后将生成的车钥匙JWE数据返回车主App。

车主App携带车钥匙JWE数据调用addPass接口跳转钱包开卡预览页，点击“立即添加”后进入激活车钥匙过程并完成车钥匙开通，车主App可以通过addPass接口的返回值接收开通结果。
