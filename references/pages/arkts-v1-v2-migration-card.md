...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-v1-v2-migration-card_
...
入口组件移除LocalStorage，仅组件内部使用的title迁移为@Local；需要跨组件共享的detail迁移为@Provider，子组件通过@Consumer同步数据。updateForm推送数据的key为detail，与@Provider的变量名一致，数据更新后通过@Provider/@Consumer自动同步给子组件。
