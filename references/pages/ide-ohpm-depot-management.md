...
从ohpm-repo 6.0.0版本开始，发布策略设置发生变化，支持分别设置标准版本发布策略和先行版本发布策略。标准版本和先行版本的定义具体请参考版本号格式。
...
点击“+”图标按钮将新增一个仓库，可以对仓库的Name、Uplink、可读策略、可写策略、发布策略和描述信息进行编辑。可读策略默认为公开可读，可写策略默认为公开可写，标准版本发布策略默认为禁止同版本覆盖，先行版本发布策略默认为禁止同版本覆盖。最多支持创建20个仓库。
