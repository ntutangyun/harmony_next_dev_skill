...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/agent-overview_
...
连接建立：系统应用（Agent客户端）连接目标智能体（Agent服务端）的AgentExtensionAbility组件，建立通信通道。

A2A通信：连接建立后，客户端和服务端通过标准化的接口进行双向数据通信。客户端发送请求，服务端在onData()回调中接收并处理数据，然后通过AgentHostProxy的sendData()方法将数据发送给客户端。

安全认证（可选）：客户端和服务端支持双向安全认证，确保通信双方身份可信。客户端发起认证，服务端在onAuth()回调中处理认证请求，然后通过AgentHostProxy的authorize()方法向客户端回复认证。
