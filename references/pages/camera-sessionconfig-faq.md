...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-sessionconfig-faq_
...
beginConfig失败，检查session是否已经被释放，或者session是否正在进行commitConfig操作。

在调用addInput接口前，可先调用canAddInput接口，验证当前session是否可以添加input。

在调用addOutput接口前，可先调用canAddOutput接口，验证当前session是否可以添加output。

在commitConfig时，需要保证上述过程没有失败，并且session在当前过程中，没有被释放或者进行其他操作。
