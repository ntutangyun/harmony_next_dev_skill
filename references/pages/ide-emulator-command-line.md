...
必选参数，指定下载的产品类型，可通过查询镜像获取deviceType，不区分大小写。
...
必选参数，指定下载的版本，可通过查询镜像返回的SoftWareVersion或osVersion获取版本号。
...
可选参数，指定模拟器的设备型号，如"Mate 70 Pro"，支持的设备型号可通过screenProfileList命令查询。如果不指定，默认使用当前产品类型最新的设备型号。
