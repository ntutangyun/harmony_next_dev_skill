...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/start-overview_
...
FA（Feature Ability）模型： HarmonyOS API 7开始支持的模型，已经不再主推。FA模型开发可见FA模型开发概述。快速入门章节不再详细说明此模型。
...
请下载并安装最新版DevEco Studio，具体可参照下载与安装DevEco Studio，更多工具使用指导可见工具概述。

完成上述操作及基本概念的理解后，可参照构建第一个HarmonyOS应用（ArkTS）进行下一步体验和学习。
