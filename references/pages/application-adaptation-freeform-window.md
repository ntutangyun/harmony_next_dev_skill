...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-adaptation-freeform-window_
...
应用通过setWindowLimits()设置自由窗口尺寸限制。

应用使用startAbility()接口拉起主窗口时通过StartOptions指定自由窗口尺寸限制。

使用module.json5配置文件中的abilities标签限制自由窗口的最大和最小尺寸。
...
标题栏是指窗口左上角的应用图标、标题以及与窗口等宽的顶部区域。
...
窗口三键是指窗口右上角的窗口最大化/还原、窗口最小化和关闭窗口三个按钮。
...
通过setWindowDecorVisible()接口隐藏标题栏。

通过getTitleButtonRect()接口或on('windowTitleButtonRectChange')接口获取到窗口三键的位置和尺寸，在页面顶部元素布局时，去除窗口三键的区域。
