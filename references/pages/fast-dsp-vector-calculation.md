...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/fast-dsp-vector-calculation_
...
根据数据类型选择对应的函数（单精度无后缀，双精度带D后缀）。

调用向量计算函数，注意设置正确的stride参数（连续存储时stride为1）。
