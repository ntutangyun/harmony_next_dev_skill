...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/notification-overview_
...
当应用进程处于运行时，开发者可以使用Notification Kit向用户发布通知。当应用进程终止后，本地通知发布通道关闭，开发者需要接入Push Kit进行云侧离线通知的发布。
...
应用发布的云端通知与本地通知都需遵守Push Kit的消息分类标准，如出现分类错误，违反消息分类标准的场景，将被判为违规。本地通知消息分类标准请参见通知渠道类型说明。

违规行为及相应的处理措施请参见违规分类、违规行为及违规处罚标准。

与相关Kit的关系

Notification Kit创建的通知会即时显示在通知中心等系统入口。如果开发者希望在应用退到后台或进程终止后仍然有一些提醒用户的定时类通知，例如购物类应用抢购提醒等，可通过Background Tasks Kit创建。目前支持基于倒计时、日历、闹钟等类型的通知提醒功能。

开发者可通过Ability Kit的getWantAgent接口设置用户点击通知后的行为意图。
