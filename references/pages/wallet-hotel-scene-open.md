...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/wallet-hotel-scene-open_
...
开发者服务器调用钱包服务器进行预置模板，将一些公共信息写入模板用于后面关联酒店房卡实例。

收到开发者端侧的申请请求之后，由开发者服务器申请酒店房卡，然后将生成的酒店房卡JWE数据返回开发者端侧。

激活酒店房卡过程中，开发者服务器收到钱包App（经过钱包服务器代理）的获取酒店个人化数据请求，返回结果给钱包App。

当酒店房卡开通成功之后，开发者服务器通过NFC相关事件回调通知接口接收到开通成功回调通知（eventType：RECEIVE_CARD）。

当用户已开通的酒店房卡数据变更（如更换新的房间号），开发者服务器需要通知钱包服务器进行酒店房卡数据更新。钱包端侧需要申请最新酒店房卡数据，钱包服务器会向开发者服务器重新触发获取酒店个人化数据请求，获取结果后将激活和更新数据写入安全芯片。

客户端开发

用户进入开发者提供的管理页面时，通过canAddPass接口检测当前设备是否支持开通酒店房卡，具体如下：
...
检测到当前设备支持开通酒店房卡后，展示开通按钮，引导用户开通酒店房卡到钱包。
...
开发者客户端携带设备信息请求开发者服务器，由开发者服务器申请酒店房卡，然后将生成的JWE数据返回客户端。
