...
从API version 24开始支持在限定宽高区域内排版文本，获取排版结果。
...
从API version 24开始支持根据坐标获取字符位置信息。

使用OH_Drawing_TypographyGetCharacterPositionAtCoordinateWithBuffer获取指定编码类型下的字符位置。返回的OH_Drawing_PositionAndAffinity包含位置索引和亲和度信息。
...
从API version 24开始支持字形范围与字符范围的相互转换。
