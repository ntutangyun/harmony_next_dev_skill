...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/sample-server-guidelines_
...
[h2]编写server_process.c、main.cpp。
...
确认服务器启动成功：执行启动命令后，服务器进程会在后台运行，可通过ps -ef | grep testserver命令查看进程状态。

验证SSL通信：确保serverKey.pem和serverCert.cer证书文件路径正确，证书文件应放置于/data/sdcard目录下。

检查端口监听：示例服务器默认监听端口需与设备端搜包接口配置一致，可通过netstat -an | grep <端口号>确认端口监听状态。

日志查看：服务器运行日志可通过logcat或串口日志查看，关键接口调用会打印[ServerProcess]前缀的日志信息。
