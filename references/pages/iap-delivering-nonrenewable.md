...
应用客户端向IAP Kit发起queryPurchases请求，获取用户已购未发货的非续期订阅商品的购买数据。

在请求参数QueryPurchasesParameter中指定对应的productType为NONRENEWABLE，同时指定queryType为iap.PurchaseQueryType.UNFINISHED。当接口请求成功时，IAP Kit将返回一个QueryPurchaseResult对象，该对象包含承载了订单信息的PurchaseData的列表。
...
如果开发者在发起购买时支持非续期订阅商品的批量购买，则需要在发货时校验下单的商品数量和PurchaseOrderPayload.quantity是否一致，避免造成漏发、多发的情况。
