...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview_
...
高精几何重建：高精几何重建主要包含稠密点云绘制、体积测量、空间识别三大能力。

[h2]人体骨骼识别与跟踪能力

人体骨骼点识别与跟踪：识别环境中的人体骨骼点信息。
...
图像跟踪：识别环境中已预置在AR Engine中的图像并输出图像位置和姿态。
...
在调用AR Engine能力前，需要先通过canIUse查询您的目标设备是否支持SystemCapability.AREngine.Core系统能力。

支持的设备类型：Phone、Tablet。从6.1.0(23)版本开始，新增支持TV。

不同设备支持的特性范围有所差异，可以通过以下方式查询设备是否支持对应的特性：

对于C API，使用HMS_AREngine_CheckSupported拓展特性查询接口进行查询。

对于ArkTS API，使用arViewController.isARTypeSupported拓展特性查询接口进行查询。
