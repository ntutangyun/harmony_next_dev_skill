...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-operator-json-configuration_
...
data_script	-	string	数据生成脚本(python)，用于生成输入和标杆数据。	
...
说明： 若无数据生成脚本，填写空字符或null。
	...
	否
gen_data	-	bool	是否根据data_script生成输入和标杆数据。	
...
说明： 算子json配置时dtype仅允许输入一种数据类型，不支持多种数据类型。
	...
	是
params	param_type	string	标识该节点是输入或输出。	
...
- 当输入为Scalar时，data_file字段删除，只需配置data_value。
	...
	否
params	data_value	由dtype确定	输入的Scalar值。	
...
说明： 仅核函数直调工程场景需要设置kernel文件相关的配置。
...
当算子为原地算子(in-place op)时，输入和输出地址一样，配置算子json时除了“param_type”和“data_file”不同，其他配置项均保持一致。配置示例如下。
