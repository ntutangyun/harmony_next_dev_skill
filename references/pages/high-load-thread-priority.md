...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/high-load-thread-priority_
...
与ArkTS端taskpool.Priority的线程优先级类似，QoS提供的优先级等级也都会相对应的映射到内核的优先级上。不过QoS提供的等级更多，自适应调度策略更强，它们属于两套不同的逻辑。

Function Flow Runtime Kit概述的QoS提供了ffrt_qos_inherit（-1）到ffrt_qos_user_initiated（3）5个优先等级，它与当前的QoS接口有着同一套底层逻辑。两者的差别在于，当前开发所用的QoS接口是直接开放给应用线程的，而FFRT的QoS则是面向任务的优先级配置，关于线程编程模型和任务编程模型的对比详见Function Flow Runtime Kit概述。
...
2、实现高、低QoS等级计算线程（关键线程）所要完成的计算任务（斐波那契数列计算）。先通过OH_QoS_SetThreadQoS接口设置当前线程的QoS等级，再执行DoFib()斐波那契数列计算。
