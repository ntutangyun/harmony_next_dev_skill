...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-rsa-sign-sig-verify-recover-pkcs1_
...
调用cryptoFramework.createSign，指定字符串参数'RSA1024|PKCS1|NoHash|OnlySign'，创建非对称密钥类型为RSA1024、填充模式为PKCS1、签名模式为OnlySign的Sign实例，用于完成仅签名操作。
...
调用cryptoFramework.createVerify，指定字符串参数'RSA1024|PKCS1|NoHash|Recover'，与签名的Sign实例保持一致。创建Verify实例，用于完成验签操作。

调用Verify.init，使用公钥（PubKey）初始化Verify实例。

调用Verify.recover，对数据进行签名恢复。
