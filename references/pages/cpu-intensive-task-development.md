...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cpu-intensive-task-development_
...
创建TaskGroup，通过addTask()添加对应的任务，然后通过execute()执行任务组，并指定为高优先级。在当前任务组所有任务结束后，会将直方图处理结果同时返回。
...
CpuIntensiveTaskDevelopment.ets
使用Worker进行长时间数据分析
