...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-avscreencapture-for-file_
...
从API version 22开始，在PC/2in1设备上对应用进行录屏时，可通过申请权限ohos.permission.CUSTOM_SCREEN_RECORDING，实现在录屏时不再弹出隐私告警弹窗。配置方式请参见受限开放权限。
...
创建AVScreenCapture实例g_avCapture。
...
调用OH_AVScreenCapture_StartScreenRecording()方法开始进行录屏。

result = OH_AVScreenCapture_StartScreenRecording(g_avCapture);

调用OH_AVScreenCapture_StopScreenRecording()方法停止录制。

result = OH_AVScreenCapture_StopScreenRecording(g_avCapture);

调用OH_AVScreenCapture_Release()方法销毁实例，释放资源。
