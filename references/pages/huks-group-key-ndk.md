...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/huks-group-key-ndk_
...
初始化密钥属性集。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP、OH_HUKS_TAG_KEY_OVERRIDE，避免密钥被覆盖。
...
调用OH_Huks_InitParamSet指定算法参数配置。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP。
...
调用OH_Huks_DeleteKeyItem删除密钥，具体请参考密钥删除。
...
密钥生成时，指定参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于生成群组密钥。

导出密钥

设备A、B导出非对称密钥对的公钥材料，具体请参考密钥导出。

导出密钥时，指定参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于导出群组密钥。

密钥协商

设备A、B分别基于本端私钥和对端设备的公钥，协商出共享密钥。

密钥协商时，指定参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于协商群组密钥。

删除密钥

当密钥废弃不用时，设备A、B均需要删除密钥，具体请参考密钥删除。

删除密钥时，指定参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于删除群组密钥。
...
获取密钥别名，指定对应的属性参数HuksOptions，添加参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于派生群组密钥。
...
调用OH_Huks_FinishSession结束密钥会话，完成派生。
