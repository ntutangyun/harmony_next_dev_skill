...
页面级弹窗功能需要结合弹窗组件的页面级能力使用，当前支持页面级能力的有Dialog和bindSheet。
...
上表列出了各类弹窗的适用场景。实际开发中，在使用自定义弹窗能力时，还需考虑侧滑拦截、点击外部关闭、自定义动画、页面切换后弹窗保留、键盘避让等交互特性，不同弹窗方案对这些特性的支持程度各异。推荐阅读自定义弹窗选型与开发，该文对比了UIContext下面的openBindSheet、openCustomDialog、getOverlayManager弹窗与Navigation.Dialog的能力差异，并提供了隐私弹窗、进度弹窗等常见场景的实现方案，帮助开发者完成弹窗选型与开发。
