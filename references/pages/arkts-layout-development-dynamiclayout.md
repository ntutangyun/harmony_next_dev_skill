...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-development-dynamiclayout_
...
布局算法类使用@ObservedV2装饰，不支持@State装饰器。

切换布局算法时，子组件的状态（如输入框内容、滚动位置等）保持不变。

在自定义布局算法的onMeasure和onLayout方法中不允许修改状态变量，避免不可预期的行为。
