...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-ndk-avrecorder-for-audio-recording_
...
开发者通过引入avrecorder.h、avrecorder_base.h和native_averrors.h头文件，使用音频录制相关API。
...
重置录制状态，调用OH_AVRecorder_Reset()重新进入idle状态，允许重新配置录制参数。

OH_AVErrCode err = OH_AVRecorder_Reset(g_recorder);

释放录制资源，调用OH_AVRecorder_Release()进入released状态，退出录制。
