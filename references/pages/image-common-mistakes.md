...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-common-mistakes_
...
异步操作的生命周期管理： 在调用Image Kit的异步接口（如packToData、packToFile、createPixelMap等）时，必须确保传入的资源对象（如PixelMap、ImageSource）在异步操作完成之前不被释放或修改。
