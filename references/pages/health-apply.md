...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/health-apply_

申请运动健康服务前，请先参考应用开发准备，确认开发环境并完成创建项目、创建HarmonyOS应用、添加公钥指纹等基本准备工作，再继续进行以下开发活动。

登录开发者联盟网站，单击进入“管理中心”。

若您尚未注册开发者账号，请先完成注册账号和实名认证。开发者可实名认证为个人开发者或者企业开发者，认证前，请先了解二者的权益区别。

在应用服务中，单击Health Service Kit卡片。如果无“Health Service Kit”卡片，请单击右上角“自定义桌面”添加卡片。

说明
暂不支持团队账号下的成员账号独立使用运动健康开发服务，详情请参见团队账号。

单击申请Health Service Kit服务，同意协议后，进入数据权限申请页面。

产品类型选择HarmonyOS应用，并填写申请信息，勾选产品必需申请的数据权限。
...
如果提交的材料不满足要求，审批将不能通过，请您根据短信或邮件通知中的驳回原因进行修改并重新提交。重新提交的审核周期约为15个工作日，如有其他疑问，请通过智能客服反馈。
...
当前审核通过仅以开发测试为目的，测试阶段有用户数量的限制，仅前100位用户可使用您申请应用中的华为运动健康服务。为解除用户数量的限制，请在应用完成开发测试验证后提交验证申请，具体请参见申请验证获取正式权限提交验证申请。

测试权限开通后，请于半年内完成申请验证获取正式权限操作，否则平台将关闭您已开通测试权限。
...
以上企业实缴注册资本数据以国家企业信用信息公示系统中最新年报数据为准。
...
章程有要求的，按照章程要求提供决策文件，如《股东（大）会决议》、《董事会决议》等。
章程无要求的，提供过半数表决权股东签字/盖章的《股东（大）会决议》。
...
通过华为开发者联盟的智能客服获取快速帮助。

通过在线提单获取人工帮助。

通过华为开发者论坛的运动健康板块寻查答案或提问。
