...
调用PubKey.getEncoded获取KeyPair中公钥对象的二进制数据。

调用PriKey.getPubKey从私钥中获取公钥对象为PubKey。

调用PubKey.getEncoded获取PubKey对象的二进制数据。
