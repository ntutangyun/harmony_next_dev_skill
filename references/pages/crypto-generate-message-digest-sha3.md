...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-generate-message-digest-sha3_
...
调用cryptoFramework.createMd，指定摘要算法SHA3-256，生成摘要实例（Md）。

调用Md.update，传入自定义消息，进行摘要更新计算。单次update长度没有限制。

调用Md.digest，获取摘要计算结果。

调用Md.getMdLength，获取摘要计算长度，单位为字节。
...
传入自定义消息，将一次传入数据量设置为20字节，多次调用Md.update，进行摘要更新计算。
