...
如果进程通过hiappevent接口在某故障事件配置策略中，使能页面切换日志，例如崩溃事件配置策略，当应用发生该故障时，hiappevent会将对应的页面切换日志快照文件返回给应用用于故障分析，日志快照文件格式如下所示：
...
实例索引：多个进程共用沙箱场景（例如多实例或ExtensionAbility组件）会出现日志写冲突，所以页面切换日志会为每个进程创建一组日志文件，使用实例索引用于标识不同进程。取值范围：[1, 10]。
