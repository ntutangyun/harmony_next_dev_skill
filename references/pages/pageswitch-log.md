...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/pageswitch-log_
...
应用调用HiAppEvent接口添加订阅故障事件。

应用调用HiAppEvent接口使能故障事件的页面切换日志。

HiAppEvent设置页面切换日志模块的使能状态。

写入页面切换日志记录关键信息。

依据使能状态决定是否写入记录应用页面切换轨迹的日志信息。
...
HiAppEvent依据使能与否向页面切换日志模块请求页面切换日志信息。

页面切换日志模块收到HiAppEvent请求后，将生成的页面切换日志快照的文件路径返回给HiAppEvent。
...
HiAppEvent将页面切换日志快照文件路径添加进故障事件的信息中，随回调一同返回给应用。
...
实例索引：多个进程共用沙箱场景（例如多实例或ExtensionAbility组件）会出现日志写冲突，所以页面切换日志会为每个进程创建一组日志文件，使用实例索引标识不同进程。取值范围：[1, 10]。
...
页面切换日志至多支持5个不同进程，每个进程至多支持10个实例，每个实例至多创建1组共2个文件。
...
页面切换日志文件数量上限为100个，达到上限时按文件修改时间先后顺序进行老化删除。
...
页面切换日志的获取当前只支持崩溃事件、应用冻屏事件、资源泄漏事件、地址越界事件。

日志获取

获取故障页面切换日志需要对故障事件进行订阅并使能故障事件的页面切换日志，日志获取依赖于HiAppEvent的事件订阅和系统事件策略配置能力。接口的详细使用说明（参数限制、取值范围等）请参考@ohos.hiviewdfx.hiAppEvent。
