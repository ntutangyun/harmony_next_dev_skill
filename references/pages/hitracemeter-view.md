...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hitracemeter-view_
...
一条有效且完整的同步跟踪trace与异步跟踪trace，均包含一条起始trace和一条结束trace，开发者需要保证接口调用的匹配性，否则会导致性能跟踪异常问题。具体可参考ArkTS接口@ohos.hiTraceMeter (性能打点)或C++接口trace.h。
...
系统侧存在一条用户态trace隶属于多个Tag的情况，ArkTS接口@ohos.hiTraceMeter (性能打点)和C++接口trace.h提供的应用侧用户态trace打点功能无法指定所属Tag集合，对应Tag分类固定为app，字段值为62。
...
如果用户态trace格式中间字段对应接口参数传入内容为空，用户态trace格式将保留|分隔符，如果尾部字段为空，用户态trace格式将不保留|分隔符。以hiTraceMeter.startAsyncTrace()和hiTraceMeter.startSyncTrace()接口的自定义聚类名称和自定义键值对参数为例，对应的格式参考如下。
