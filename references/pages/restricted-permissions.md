...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/restricted-permissions_
...
应用读取自身写入到图库的图片/视频文件时，无需使用读取权限。

用户在应用内手动选择图片/视频并进行处理，如：
...
无需权限的建议方案： 使用安全控件或授权弹窗的方式，将用户指定的媒体资源保存到图库中，使用方式请参考：保存媒体库资源。
...
在API 12及以上版本，该权限等级变更为normal，应用可直接声明使用；若需兼容API12之前版本，仍需按受限权限申请方式使用该权限。
