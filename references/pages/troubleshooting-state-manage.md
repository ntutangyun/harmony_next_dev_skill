...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/troubleshooting-state-manage_
...
在上面的示例中，Inner没有被@Observed装饰，所以其属性value的赋值无法被观察：
...
依赖其所属组件的更新函数：如@State通知@Prop变化、@State通知@ObjectLink变化。开发者可以使用断点调试工具，或者getHash接口来判断数据源和同步对象是否为同一个对象的引用(hashcode并不固定，以开发者自己打印的为准)。
