...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/xengine-kit-ai-temporal-upscaling_
...
在游戏初始化阶段，调用HMS_XEG_GetString接口查询XEngine Kit支持的特性列表。检查返回列表中是否包含XEG_TEMPORAL_UPSCALE_EXTENSION_NAME。若不包含，则当前设备不支持此特性，流程终止。

调用HMS_XEG_TemporalUpscaleParameter接口配置超分相关参数。

游戏运行时，首先渲染待超分的当前帧纹理。此阶段需完成包含Jitter的主Pass渲染，并确保Depth、Motion Vector和Color等输入纹理已准备就绪。

当待超分纹理渲染完成后，调用HMS_XEG_RenderTemporalUpscale接口对纹理执行时域AI超分处理。

超分完成后，继续渲染剩余纹理，如UI等。全部渲染结束后，进行帧送显。

游戏退出时，XEngine Kit会自动释放超分相关资源，无需手动管理。

下面是基于Vulkan图形API平台集成时域AI超分的主要业务流程

用户进入游戏初始化场景时，调用HMS_XEG_EnumerateDeviceExtensionProperties接口查询XEngine Kit支持的特性列表。检查返回列表中是否包含XEG_TEMPORAL_UPSCALE_EXTENSION_NAME。若不包含，则当前设备不支持此特性，流程终止。

调用HMS_XEG_CreateTemporalUpscale接口创建时域AI超分实例。

游戏运行过程中，渲染当前待超分的帧纹理。

待超分纹理渲染完成（即带jitter的主pass渲染结束，且depth、motion vector、color等输入纹理准备就绪）后，调用HMS_XEG_CmdRenderTemporalUpscale接口执行超分处理。

超分渲染完成后，继续渲染剩余纹理（如UI等），渲染结束后进行画面送显。

游戏退出时，调用HMS_XEG_DestroyTemporalUpscale接口销毁超分实例。
...
若需使用Vulkan时域AI超分特性，请引用XEngine Kit的CMakeLists，CMakeLists.txt部分示例代码如下，完整示例代码请参见Demo（GPU加速引擎-Vulkan）。
...
调用HMS_XEG_GetString接口，获取XEngine支持的扩展信息，只有在支持XEG_TEMPORAL_UPSCALE_EXTENSION_NAME扩展时才可以使用时域AI超分的相关接口。
...
调用HMS_XEG_TemporalUpscaleParameter接口，对时域AI超分的参数赋值。
...
调用HMS_XEG_RenderTemporalUpscale接口进行超分，每帧都需要调用。
...
调用HMS_XEG_EnumerateDeviceExtensionProperties接口，获取XEngine支持的扩展信息，只有在支持XEG_TEMPORAL_UPSCALE_EXTENSION_NAME扩展时才可以使用时域AI超分的相关接口。
...
调用HMS_XEG_CreateTemporalUpscale接口，创建时域AI超分实例。
...
调用HMS_XEG_CmdRenderTemporalUpscale接口下发超分，每帧都需要调用。
...
调用HMS_XEG_DestroyTemporalUpscale接口销毁实例。
