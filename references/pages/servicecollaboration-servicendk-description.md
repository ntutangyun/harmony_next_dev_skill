...
基于分布式协同框架面向跨设备拍照的业务场景，为您提供了 HMS_ServiceCollaboration_GetCollaborationDeviceInfos（设备列表接口）、HMS_ServiceCollaboration_StartCollaboration（跨端拍照、扫描、拉起图库选择图片）或HMS_ServiceCollaboration_StartCollaborationV2（跨端拍照、扫描、拉起图库选择图片和视频）和 HMS_ServiceCollaboration_StopCollaboration（终止跨设备互通）四个接口。只需要调用这四个接口，即可完成跨设备互通，无需关注分布式场景下数据传输、指令控制等具体细节。
...
拍摄状态可能为：对端设备拍摄中、图片导入中、协同失败、本端WLAN未开启、双端WLAN或者蓝牙未开启。具体拍摄状态提示可由应用选择绘制，对应提示信息参考ServiceCollaborationEventCode。
