...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-events-smartgesture-event_
...
使能手势：页面出现时调用enableSmartTapAndSlideGestures启用敲一敲和划一划手势。

标记组件：为需要响应智慧手势的组件设置id和smartGestureShortcut属性，声明其响应优先级和可选中能力。

注册监听：调用registerMonitor注册监听回调，在系统处理手势前接收手势意图并进行自定义决策。如需取消单个已注册的回调，可调用unregisterMonitor注销指定回调。

动态决策：在监听回调中，根据回调返回的具体操作类型ActionProposal和用户实际的操作意图，构造合适的ActionProposal，通过GestureHandlingResolution返回处理结果。

关闭手势：页面消失时调用clearMonitors清空所有监听回调（也可通过unregisterMonitor逐个注销指定回调），并调用enableSmartTapAndSlideGestures关闭手势使能。
