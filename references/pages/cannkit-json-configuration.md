...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-json-configuration_
...
data_script	N/A	string	数据生成脚本（python），用于生成输入和标杆数据。	
...
说明： 若无数据生成脚本，填写空字符或null。
	...
	否
gen_data	N/A	bool	是否根据data_script生成输入和标杆数据。	
...
outputs
	...
	data_value	由dtype确定	输入的Scalar值。	
...
outputs
	...
	value_depend	string	输入是否为Const类型	

该配置项可选，不配置该字段表示输入不为Const类型。
...
当算子为原地算子(in-place op)时，输入和输出地址一样，配置算子json时“outputs”和“inputs”除了“data_file”不同，其他配置项均保持一致。配置示例如下。
