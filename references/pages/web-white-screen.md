...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-white-screen_

Web页面出现白屏的原因众多，本文列举了若干常见白屏问题的排查步骤，供开发者快速定位。

首先排查权限和网络状态。

通过使用DevTools工具调试前端页面定位具体报错类型（跨域、资源404、JS异常）。
...
检查是否开启坚盾守护模式，坚盾守护模式开启后相关限制见：ArkWeb限制的HTML5特性。
...
检查控制台，确认是否存在因MixedContent策略或CORS策略导致的异常，或JS错误等。可参考解决Web组件本地资源跨域问题。为了提高安全性，ArkWeb内核禁止file协议和resource协议访问跨域请求。因此，在使用Web组件加载本地离线资源的时候，Web组件会拦截file协议和resource协议的跨域访问。Web组件无法访问本地跨域资源时，DevTools控制台会显示报错信息：
...
异步渲染模式下（renderMode: RenderMode.ASYNC_RENDER），Web组件的宽高不能超过7,680px（物理像素），超过会导致白屏。
...
通过setRenderProcessMode设置WebView渲染模式为单进程加载。
