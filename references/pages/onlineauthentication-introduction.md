...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/onlineauthentication-introduction_
...
通行密钥（Passkey）是基于FIDO2标准协议（见网站链接免责声明）实现的一种简单又安全的登录方式。借用通行密钥，用户可使用指纹、人脸或手机解锁PIN码登录应用。相较于传统密码，通行密钥具有更便捷、安全的优势。
...
要使用指纹或3D人脸的免密身份认证能力，移动端设备需要支持相应的生物特征，查询当前移动端设备是否支持可参见User Authentication Kit（需设备支持ATL4级别的认证可信等级）。
