...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/huks-import-key-in-plaintext-ndk_
...
封装密钥属性集和密钥材料。通过OH_Huks_InitParamSet、OH_Huks_AddParams、OH_Huks_BuildParamSet构造密钥属性集paramSet。

密钥属性集中必须包含OH_Huks_KeyAlg、OH_Huks_KeySize、OH_Huks_KeyPurpose属性。

密钥材料须符合HUKS密钥材料格式。

调用OH_Huks_ImportKeyItem，传入密钥别名和密钥属性集，导入密钥。
