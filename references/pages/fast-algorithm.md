...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/fast-algorithm_
...
构造HMS_FAST_SortData结构体。

调用HMS_FAST_Algo_Sort进行排序。
...
指定排序区间参数（offset和count）。

调用HMS_FAST_Algo_PartialSortAt进行部分排序。
...
构造HMS_FAST_SortData结构体（注意sizeOf必须为sizeof(char*)）。

指定排序方向（ascend：非零为升序，零为降序）。

调用HMS_FAST_Algo_NaturalSort进行自然语言排序。
