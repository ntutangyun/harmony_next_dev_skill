...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cross-language-complex-param_
...
函数声明与调用，与arrayBuffer类型数据交互类同，此处不再赘述。
...
在DevEco Studio预生成的Init()函数中设置写好的数据传递函数。与arrayBuffer类型数据交互类同，此处不再赘述。
...
三方库获取：HarmonyOS / third_party_json · GitCode
...
需在DevEco Studio预生成的Init()函数中设置写好的数据传递函数。与arrayBuffer类型数据交互类同，此处不再赘述。
...
value2String、string2value()：负责napi_value与string类型之间的相互转换，实现方法详见hashMap类型数据交互。
...
string2Value()：负责将string转换为napi_value数据。此函数的实现方法详见hashMap类型数据交互。
...
value2String()：负责将入参转换为string。此函数的实现方法详见hashMap类型数据交互。
