...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-skill-scenes_
...
客户端Skill：生成推送通知消息、后台消息、应用内通话消息的ArkTS代码以及配置工程依赖。

服务端Skill：生成服务端工程代码，实现Push场景化消息发送能力。

[h2]DevEco CodeGenie接入示例

打开DevEco Studio之后，在右侧工具栏点击CodeGenie，详情见DevEco CodeGenie工具概述。

参考操作步骤导入Skill。如图所示，选择在下载并配置Skill中下载的Skill，需要导入名为hmos-push-kit的文件夹，以及4个子文件夹。

全部导入成功后，如图所示。

回到对话页面，选择HarmonyOS Act以及合适的模型，并按需求输入提示词。如图所示，CodeGenie调用对应技能接入Push Kit。

接入完成后，CodeGenie会提示需要开通推送服务等注意事项。
