...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-rotation_
...
应用可以通过在model.json5配置文件中配置"orientation"字段或者在运行时调用setPreferredOrientation()接口设置"orientation"字段，设置应用的显示方向。无论是哪种方式设置的orientation字段含义都一样，即旋转显示方向类型枚举（也叫旋转策略）。详情可参考设置窗口的旋转策略。
