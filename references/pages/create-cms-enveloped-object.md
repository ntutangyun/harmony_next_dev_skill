...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/create-cms-enveloped-object_
...
调用cert.createCmsGenerator创建cmsGenerator对象。

调用cmsGenerator.setRecipientEncryptionAlgorithm设置加密算法。

调用cmsGenerator.addRecipientInfo添加接收者信息。

调用cmsGenerator.doFinal获取CMS最终封装数据。

调用cmsGenerator.getEncryptedContentData获取CMS封装密文数据。
