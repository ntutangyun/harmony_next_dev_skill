...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-convert-compressed-or-uncompressed-ecc-point-ndk_
...
指定uint8_t类型的ECC非压缩点数据，调用OH_CryptoEcPoint_Create，构造OH_CryptoEcPoint对象，用于生成点数据。

调用OH_CryptoEcPoint_Encode，获取压缩点数据。
...
指定uint8_t类型的ECC压缩点数据，调用OH_CryptoEcPoint_Create，构造OH_CryptoEcPoint对象，用于生成点数据。

调用OH_CryptoEcPoint_Encode，获取非压缩点数据。
