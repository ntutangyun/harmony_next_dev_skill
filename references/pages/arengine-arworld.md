...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-arworld_

本章节给出了关键开发步骤，完整代码可以参考示例代码。

约束与限制

从5.1.0(18)开始，物体摆放能力支持部分Phone、部分Tablet设备。请参考硬件要求判断设备是否支持运动跟踪及平面识别特性（ARENGINE_FEATURE_TYPE_SLAM）。

接口说明

AR物体摆放主要依赖ARViewContext，以下接口为AR物体摆放相关接口。详细接口和说明，请参考AR Engine API参考。
...
调用ARViewCallback，使用其中的onFrameUpdate方法进行帧数据更新，获取平面信息。
