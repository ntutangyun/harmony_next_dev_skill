...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/notification-subscriber-extension-ability-development-steps_
...
实现NotificationSubscriberExtensionAbility后，还需要在合适的时机调用openSubscriptionSettingsWithResult接口，打开通知扩展订阅设置页面，引导用户授予获取本机通知的权限，该页面以半模态弹窗显示，并在弹窗关闭时返回授权的结果。建议在设备管理页面提供一个通知授权的按钮，用户点击按钮则调用openSubscriptionSettingsWithResult接口。
...
用户收到消息后，如果蓝牙连接是无效的，则需建立蓝牙连接。

假如蓝牙连接已经存在，则直接使用此连接发送消息。

如果使用该蓝牙连接发送消息失败，则需要重新建立蓝牙连接，连接成功后即可发送消息。
