...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-config-prompt-template_

提示词模板采用结构化的固定格式，将“角色、任务、输入和输出要求”等要素清晰组织起来，使开发者只需填充关键信息，即可稳定、高效地引导模型生成预期结果。开发者可以通过在指定应用资源目录放置prompt_template.json配置文件，导入自定义提示词模板。
...
在应用资源目录的rawfile/arkdata/rag下创建prompt_template.json文件，若工程中无目标目录则递归创建。
