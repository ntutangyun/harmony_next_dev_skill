...
调用cameraManager中的createSession方法创建一个会话。
...
调用VideoSession中的beginConfig方法配置会话。
...
会话控制。调用VideoSession中的stop方法可以停止当前会话。调用removeOutput和addOutput方法可以完成会话切换控制。以下示例代码以移除拍照流photoOutput，添加视频流videoOutput为例，完成了拍照到录像的切换。
