...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-components-text-input_
...
TextInput有以下类型可选择：Normal基本输入模式、Password密码输入模式、Email邮箱地址输入模式、Number纯数字输入模式、PhoneNumber电话号码输入模式、USER_NAME用户名输入模式、NEW_PASSWORD新密码输入模式、NUMBER_PASSWORD纯数字密码输入模式、NUMBER_DECIMAL带小数点的数字输入模式、URL带链接的输入模式。通过type属性进行设置：
...
输入框可以通过ellipsisMode属性设置省略位置。

ellipsisMode属性需要配合textOverflow属性设置为TextOverflow.Ellipsis使用，单独设置ellipsisMode属性不生效。
...
从API version 20开始，输入框可以通过strokeWidth和strokeColor属性设置文本的描边宽度及颜色。
...
从API version 20开始，支持通过lineSpacing设置文本的行间距。如果不配置LineSpacingOptions时，首行上方和尾行下方默认会有行间距。如果onlyBetweenLines设置为true时，行间距仅适用于行与行之间，首行上方无额外行间距。
