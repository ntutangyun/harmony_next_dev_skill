...
Navigation：当前显示的NavDestination中的自定义组件为active状态，而其他未显示的NavDestination组件则为inactive状态。需要注意，本文档中涉及的“激活（active）/非激活（inactive）”是指组件冻结的激活/非激活状态，和NavDestination组件中的onActive和onInactive不同。
...
1.在页面1中点击first page storageLink + 1，storageLink状态变量改变，@Watch注册的方法first会被调用。
