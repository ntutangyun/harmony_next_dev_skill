...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/freeform-window-overview_
...
自由多窗模式下，允许用户在一块屏幕上同时显示多个应用窗口，此时的应用窗口为自由窗口。
...
由于自由窗口支持大小缩放，且主窗口默认存在标题栏，与非自由窗口状态下存在差异。应用如果是通过屏幕宽高计算布局，未适配窗口宽高变化时，缩放窗口可能会导致界面截断、遮挡或控件叠加等问题。因此需要进行自由窗口的适配，具体可见应用适配自由窗口。
