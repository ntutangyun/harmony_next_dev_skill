...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-xmp-metadata_
...
推荐使用readImageMetadataByType接口获取XMP元数据，可以直接传入MetadataType.XMP_METADATA，精准获取XMP类型的元数据，避免读取其他无关元数据。
...
使用标准命名空间（如xmp、dc、exif等）时，无需额外注册。Image Kit支持但不限于常量中定义的标准命名空间。
...
以下示例假设已注册自定义命名空间book。如果尚未注册，请先调用registerXMPNamespace完成注册。
...
多语言文本（语言替代数组）本质上也是一种数组，类型为ALTERNATE_TEXT。在添加元素前，必须先创建数组容器；且每个元素必须通过"xml:lang"限定符指定语言，否则在序列化时会失败，导致无法写回文件。
...
该语法仅适用于enumerateTags和getTags接口的rootPath参数。
