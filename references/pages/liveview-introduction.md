...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/liveview-introduction_
...
实况窗支持HarmonyOS 5及以上的操作系统版本。

实况窗当前仅支持Phone和Tablet机型。
...
为避免在26.0.0之前旧系统版本上实况窗无法显示PixelMap类型图片，要求在新旧系统版本上传入不同大小的图片PixelMap实例进行适配，请参考应用使用API如何在不同系统版本设备上做兼容性保护判断（ArkTS/C++）。
...
应用发送的实况窗通知需同时遵守实况窗设计规范和Push Kit的通知内容原则，如出现违反实况窗设计规范、通知内容原则等行为，将被视为违规。
