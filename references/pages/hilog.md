...
使用DevEco Studio工具在线查看日志，参考：日志分析。

查看落盘的文件

log版本：hilog日志默认开启落盘，不同产品之间落盘数量可能存在差异，具体可以使用查看和设置落盘任务命令进行查询。

nolog版本：hilog日志默认不落盘，需要手动开启落盘，参考查看和设置落盘任务；设备打开开发者模式后自动开启落盘。

日志保存位置：设备data/log/hilog目录下，可以导出日志进行查看。导出及解析工具参考：hilogtool。
...
处理方式：可参考进程超限开关，关闭对应管控机制。建议每个应用进程每秒打印日志量不超过50KB，否则可能会触发日志超限，导致日志丢失。
...
处理方式：可参考domain超限开关，关闭对应管控机制。建议每个domainID每秒打印日志量不超过50KB，否则可能会触发日志超限，导致日志丢失。
