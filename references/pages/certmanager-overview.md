...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/certmanager-overview_
...
系统证书凭据：用于系统服务（如WLAN、VPN服务）连接服务器时，服务器对接入设备进行身份认证。一般由设备的用户进行安装和管理，应用可以通过API拉起证书管理服务的对话框，引导用户安装系统证书凭据。

用户证书凭据：归属于设备用户的证书凭据，由设备的用户进行安装和管理。应用可以通过API拉起证书管理服务的对话框，引导用户安装用户证书凭据。应用在使用用户证书凭据前，需要调用证书管理服务API获取用户的授权。

应用证书凭据：归属于应用的证书凭据，由应用通过证书管理服务API进行安装和管理。设备的用户不能查看和管理应用证书凭据。
