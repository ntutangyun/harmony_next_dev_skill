...
该接口仅允许在录音流处于运行态时调用，否则会返回AUDIOSTREAM_ERROR_ILLEGAL_STATE。如果同一录音流同时设置了流级静音提示和会话级静音提示OH_AudioSessionManager_SetCaptureMuteHint，流级静音提示优先级更高，以流级设置值为准。当前未提供系统查询接口，如需在界面展示静音提示状态，应用需要自行维护最近一次设置成功的状态。
