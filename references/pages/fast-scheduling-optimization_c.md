...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/fast-scheduling-optimization_c_
...
短持续时间（HMS_FAST_SHORT）：单次最大持续时间1秒，间隔大于3秒。

中等持续时间（HMS_FAST_MEDIUM）：单次最大持续时间10秒，间隔大于30秒。

长持续时间（HMS_FAST_LONG）：单次最大持续时间60秒，间隔大于180秒。

支持同时上报多个线程。可以携带线程ID数组。

约束与限制

perfHint仅作为性能优化建议，系统在综合考量整机CPU负载、系统温度、当前任务队列等因素后决定是否实施优化措施，因此不能保证每次调用都能获得性能提升。
...
perfHint系统性能优化场景提供以下C接口，具体API说明详见接口文档。
...
需要开发者引入scheduling_optimization.h头文件后，才可以使用系统性能优化相关API。

#include "FASTKit/scheduling_optimization.h"

完整示例

应用可以通过HMS_FAST_SchedulingOptimization_PerfHint接口进行系统性能优化。
