...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-build-analyzer_
...
如需查看决定构建时长的任务的占比细分数据，请点击概览页面上的Common views into this build下方链接 ，您也可以从下拉菜单中选择Tasks并确认您要的任务分组类别。任务以模块、业务类别、Target以及同一模块下的Target、同一模块下的业务类别和同一Target下的业务类别进行分组。图表中任务按照时间占比从大到小排列，点击子任务可详细了解其执行情况。
...
如果要分析构建过程的内存消耗情况，需要先设置构建分析模式为Advanced，启用内存监测，构建后会生成内存消耗曲线图。从DevEco Studio 5.1.1 Beta1版本开始支持。

点击概览页面上Common views into this build下方的链接Memory Usage During Build，也可以从下拉菜单中选择Tasks和Memory Usage分组，查看内存消耗曲线图。
