...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/testhelper-guidelines_
...
[h2]testhelper enable-location-mock
...
[h2]testhelper disable-location-mock
...
[h2]testhelper set-mocked-locations
...
[h2]服务不可用（Error: Service is not available）
...
参数格式不正确（如时间格式、文件路径格式等）。

参数值超出限制范围（如时间值、文本长度、位置数量等）。

文件路径不符合要求（如路径前缀、文件扩展名等）。
...
确保文件路径符合要求（如为/data/local/tmp下的绝对路径、拥有正确的扩展名）。
