...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-avsession-avmusictemplate_
...
在进程启动时，调用接口createAVMusicTemplate创建音频模板实例（每个媒体应用创建一个音频模板实例，不需要重复创建）并拉起音频模板。
...
注册事件监听，在监听到事件后可提供应用数据给音频模板使用。监听接口详情请查看AVMusicTemplate。
...
媒体应用启动时注册的事件监听需要在应用退出时注销，以释放资源。注销接口详情请查看AVMusicTemplate。
