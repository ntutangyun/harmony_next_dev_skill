...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-component-encapsulation_
...
使用@Component创建自定义组件，将不变的部分实现在组件内部，将可能发生变化的部分，以参数变量形式对外提供。
...
使用@Watch监听状态变量，当父组件修改此变量时，@Watch的回调方法将被执行，即实现了子组件中的方法调用。
...
使用Emitter通信机制，在子组件中添加事件监听，在父组件emit发布对应事件，监听的事件被执行，即实现了子组件中的方法调用。
