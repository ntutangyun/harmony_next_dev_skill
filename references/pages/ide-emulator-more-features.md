...
使用DevEco Studio代理：读取并应用DevEco Studio上的网络代理配置。
说明
...
应用调用对应Kit提供的API接口，通过电脑摄像头实现预览、拍照、扫码等功能。
...
添加屏幕
启动模拟器，点击工具栏的多屏按钮，打开多屏界面。

点击添加，可以选择Mate系列、Pura系列、Nova系列等产品型号，点击应用即可添加对应的屏幕。

默认情况下，所有的屏幕是整体拖动和缩放的，如需单独拖动和缩放单个屏幕，请勾选界面上的每个副屏在独立窗口中显示，并点击应用按钮。从DevEco Studio 6.0.1 Beta1版本开始支持。

如需在多个屏幕上同时启动应用，请按界面提示，将module.json5中的launchType字段配置为multiton，具体请参考UIAbility组件启动模式。
