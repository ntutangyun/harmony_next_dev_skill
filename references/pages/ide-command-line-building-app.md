...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-command-line-building-app_
...
说明
如果开发者所使用的电脑处于完全无网络的环境中，搭建构建环境请参考无网络流水线搭建。
...
获取命令行工具
命令行工具获取。其他系统(Windows/macOS)请根据实际情况下载对应版本。
...
在linux系统的构建场景下，使用纹理压缩功能需要安装libGL1库。例如Ubuntu/Debian上安装libgl1-mesa-dev，CentOS/RHEL上安装mesa-libGL-devel。
...
如果是在daemon模式下，请参考设置守护进程内存。

编译构建常见的任务和扩展参数如下，更多关于Hvigor命令行参数详见：常用命令。
...
参考配置JDK步骤配置环境变量后，打开命令行终端，执行如下命令，生成密钥库文件。例如，生成的密钥库名称为demo.p12，存储到path目录下。
...
执行如下命令，执行后需要输入storepass密码，生成证书请求文件。
...
keystore：与上一步骤中输入的keystore保持一致。
...
在签名工具目录下，使用如下命令进行签名。详细的签名工具指导请参考Hap包签名工具。
...
先配置环境变量，再打开命令行工具，在文件夹下执行 npm install 命令，会生成node_modules文件夹。
...
打开命令行工具，在文件夹下执行 npm install 命令，会生成node_modules文件夹。
