...
（2）BACKGROUND_USER_IDLE用户闲时任务锁可以阻止系统自动睡眠，但不能阻止系统强制睡眠。因此使用该接口的应用必须监听进入强制睡眠的公共事件，在接收到该公共事件后1s内主动释放掉该锁；是否监听退出强制睡眠的公共事件并重新持有锁，由应用根据具体场景自行决策。
...
申请使用运行锁所需的权限：ohos.permission.RUNNING_LOCK。申请流程请参考：申请应用权限。
