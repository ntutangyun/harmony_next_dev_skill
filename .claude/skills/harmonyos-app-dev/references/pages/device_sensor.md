...
调用wearEngine中的getSensorClient方法，获取SensorClient对象。

调用getSensorList方法，获取指定连接设备的传感器列表。
...
参见上一小节获取穿戴设备的传感器列表，获取设备的传感器列表信息。
...
传感器分为多通道传感器，如PPG、ECG，和非多通道传感器，如ACC。对于多通道传感器上报的数据，可以通过SensorData中的channel字段来区分通道。
