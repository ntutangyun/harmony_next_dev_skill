...
在修改完代码后，点击Apply Changes推送增量包安装至设备。
...
执行hdc命令安装HAP、HSP，关于hdc工具的使用指导请参考hdc。
...
开发者通过独立的构建流程，识别出希望构建增量hqf包的so，根据ABI编译环境（可查看build-profile.json5的abiFilters字段），汇总到某一目录下，例如汇总在change_test目录下，编译环境是arm64-v8a，示例如下。
...
在hqf打包工具目录下（默认在DevEco Studio安装目录\sdk\default\openharmony\toolchains\lib下），执行命令打包，示例如下。
