...
为了使用共享沙箱，需要先申请一个data-group-id，并将其分别配置到应用主入口的profile和输入法扩展的dataGroupIds中。具体流程如下：
...
待您收到data-group-id申请成功的邮件回复后，重新生成应用的profile，新生成的profile里面包含本次申请到的data-group-id；并使用DevEco Studio配置工程的签名信息，将新的profile配置到工程中。

按照data-group-id申请成功的回复邮件指导，获取到您本次申请到的data-group-id，并在InputMethodExtensionAbility所在的module.json5中配置dataGroupIds。
