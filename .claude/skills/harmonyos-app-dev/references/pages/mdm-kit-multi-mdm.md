...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/mdm-kit-multi-mdm_
...
1.通过adminManager.enableDeviceAdmin接口, 激活后成为DA角色。

2.通过adminManager.startAdminProvision接口, 激活后成为BDA角色。
...
示例1：若MDM A先设置了位置策略-强制开启位置服务策略，则MDM B尝试再设置位置策略-禁用位置服务策略将报策略冲突。

示例2：若MDM A先设置了位置策略-禁用位置服务策略，则MDM B尝试再设置位置策略-强制开启位置服务策略将报策略冲突。
