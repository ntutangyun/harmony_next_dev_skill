...
调用createMediaKeySystem和createMediaKeySession根据DRM信息中的uuid创建MediaKeySystem和MediaKeySession实例。
...
调用generateMediaKeyRequest生成媒体密钥请求，并调用processMediaKeyResponse处理媒体密钥响应。
...
调用requireSecureDecoderModule和setDecryptionConfig，在处理媒体密钥响应成功后设置解密session。
