...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/huks-group-key-arkts_
...
初始化密钥属性集。需要添加群组密钥标签HUKS_TAG_KEY_ACCESS_GROUP，推荐使用HUKS_TAG_KEY_OVERRIDE，避免密钥被覆盖。
...
使用HuksParam设置加密算法参数配置。需要添加群组密钥标签HUKS_TAG_KEY_ACCESS_GROUP。
...
使用HuksParam设置解密算法参数配置。需要添加群组密钥标签HUKS_TAG_KEY_ACCESS_GROUP。
...
使用HuksParam设置密钥删除算法参数配置。需要添加群组密钥标签HUKS_TAG_KEY_ACCESS_GROUP。

调用deleteKeyItem删除密钥，具体请参考密钥删除。
...
获取密钥别名，指定对应的属性参数HuksOptions，添加参数HUKS_TAG_KEY_ACCESS_GROUP，用于派生群组密钥。
