...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding-roi_
...
依赖ROI检测识别能力： 编码器不具备ROI的检测识别能力，所以ROI编码技术生效依赖于开发者输入的ROI信息。开发者可根据业务场景自行设计并实现ROI识别能力，或通过调用系统相机模块原生提供的人脸区域信息，降低开发成本，具体请参考相机人脸ROI获取示例。
...
配置ROI支持两种方式：NativeBuffer元数据配置方式和编码输入回调配置方式。编码输入回调配置方式包含编码输入参数回调（Surface模式）和编码输入buffer回调（Buffer模式）。
...
绘制之前获取最新的NativeBuffer，并配置ROI信息。绘制过程可参考OpenGLES示例，最终通过eglSwapBuffers送绘制好的数据到编码器进行编码。
...
同Surface模式下通过NativeBuffer接口配置ROI步骤2。

Native层解析TS层传递的人脸位置信息。

同Surface模式下通过NativeBuffer接口配置ROI步骤3。
...
视频编码的详细操作步骤请参考视频编码开发指南，下面仅针对ROI编码做具体说明。
...
在该场景中，视频帧和ROI均由应用提供，并采用buffer模式编码。开发者可以参考前文所述的基于时间戳匹配或基于回调时机匹配两种对齐方式来实现ROI与视频帧的对齐，并在编码输入Buffer回调中完成ROI参数的配置（如图4所示）。

图4：编码输入Buffer回调接口配置ROI流程图

准备步骤同Surface模式下通过编码输入回调接口配置ROI步骤1-4，此处仅说明配置差异。
