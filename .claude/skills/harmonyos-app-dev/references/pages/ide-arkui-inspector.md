...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-arkui-inspector_
...
查看窗口交互事件，包括触屏、鼠标、按键、滚轮、窗口焦点变化事件。
...
不支持应用市场上架的商用签名应用。
操作步骤
在DevEco Studio下方点击ArkUI Inspector，打开ArkUI Inspector。
...
在设备框，点击设备列表的最后一项Stop inspector，可断开与设备的连接。

显示组件信息
点击，勾选Show Tree Statistics，可显示组件树组件信息。

点击，勾选Show Hidden Components，可显示隐藏的组件。

点击，勾选Show Custom Components，可过滤自定义组件。

点击，勾选Show System Components，可过滤系统组件。
...
UI组件源码跳转
单击Run > Edit Configurations，勾选“Enable DebugLine”，点击OK保存后，重新运行工程，表示开启源码跳转功能。
...
从DevEco Studio 6.1.0 Beta1版本开始，支持查看窗口交互事件，包括触屏、鼠标、按键、滚轮、窗口焦点变化事件，帮助开发者定位窗口发生失焦、获焦、重绘等问题。

选择WindowEvents页签，点击Start按钮，开始上报事件消息，包括事件时间戳、窗口ID、事件类型、坐标等，支持按事件类型过滤。点击Stop按钮，即可停止上报事件。
