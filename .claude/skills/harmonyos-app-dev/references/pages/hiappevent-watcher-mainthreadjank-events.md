...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-mainthreadjank-events_
...
sample_count	string	否	

主线程超时采样次数。系统检测到当前主线程执行任务时长达到可采样阈值时，开始周期性采集堆栈，每个间隔采集一次堆栈，共采集sample_count次。
...
主线程超时采样次数。系统检测到当前主线程执行任务时长达到可采样阈值时，开始周期性采集堆栈，每个间隔采集一次堆栈，共采集MAIN_THREAD_JANK_PARAM_SAMPLE_COUNT次。
...
配置项名称	类型	必须配置	说明
MAIN_THREAD_JANK_PARAM_LOG_TYPE	const char*	否	
...
MAIN_THREAD_JANK_PARAM_SAMPLE_INTERVAL	const char*	否	
...
MAIN_THREAD_JANK_PARAM_IGNORE_STARTUP_TIME	const char*	否	
...
MAIN_THREAD_JANK_PARAM_SAMPLE_COUNT	const char*	否	

主线程超时采样次数。系统检测到当前主线程执行任务时长达到可采样阈值时，系统检测到当前主线程执行任务超过采样限制后，开始周期性采集堆栈，每个间隔采集一次堆栈，共采集MAIN_THREAD_JANK_PARAM_SAMPLE_COUNT次。
...
MAIN_THREAD_JANK_PARAM_REPORT_TIMES_PER_APP	const char*	否	
...
MAIN_THREAD_JANK_PARAM_AUTO_STOP_SAMPLING	const char*	否	
