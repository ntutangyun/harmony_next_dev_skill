...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/socket-connection_
...
绑定本地IP地址和端口，确保bind成功后，再订阅TLS Socket相关的订阅事件。上传客户端CA证书及数字证书，调用connect接口建立连接。连接成功后，可调用send接口发送数据。
...
import { BusinessError } from '@kit.BasicServicesKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
Tcp2TwoWayTlsWorker.ets

参考应用 TCP/UDP 协议进行通信，创建一个TCPSocket连接。
