...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-sync-of-rdb-store_
...
单版本表模式使用需要配置schema文件，用以指定同步列以及指定解冲突列，请参考使用单版本表模式进行数据同步。
...
以下是关系型设备协同分布式数据库跨设备数据同步功能的相关接口，更多接口及使用方式请见@ohos.data.relationalStore (关系型数据库)。
...
调用on('dataChange')接口监听其他设备的数据变化，当数据变化同步至当前设备时，将执行订阅的回调方法，入参为数据发生变化的设备ID列表。
...
当前设备分布式表中的数据发生变化后，调用RdbStore的sync接口传入SYNC_MODE_PUSH参数推送数据变化至其他设备。
通过谓词的inDevices方法指定推送的目标设备。
// 同步当前设备数据变化至组网内其他设备
if (store) {
  // 当前设备分布式数据表中插入新数据
...
当前设备可调用RdbStore的sync接口传入SYNC_MODE_PULL参数拉取组网内其他设备的数据变化。
通过谓词的inDevices方法指定拉取的目标设备。
// 拉取组网内其他设备的数据变化
if (store) {
  // 查询组网内的设备列表
  const deviceManager = distributedDeviceManager.createDeviceManager('com.example.rdbDataSync');
...
当数据未完成同步，或未触发数据同步时，可使用RdbStore的remoteQuery方法查询组网内指定设备上分布式表中的数据。
