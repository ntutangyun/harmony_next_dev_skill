...
在AppAnalyzer页面，选择场景化体检，选择预置的体检卡片，或根据需要自定义卡片。
...
在AppAnalyzer页面，选择场景化体检，选择自动或手动方式，模块选择框选择HarmonyOS应用/元服务工程模块。
...
勾选体检场景，不同场景对应的检测指标请参考体检场景，然后在AppAnalyzer页面底部单击开始按钮，开始测试。首次测试时，请根据AppAnalyzer的指引，下载Python及三方库，或者登录开发者账号并自动签名音频辅助检测APP。在测试过程中，请保持连接的设备为解锁亮屏状态。
说明
