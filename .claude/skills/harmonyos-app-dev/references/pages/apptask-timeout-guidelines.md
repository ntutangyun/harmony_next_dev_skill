...
_Source: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/apptask-timeout-guidelines_
...
堆栈采集	150ms < 主线程处理时长 < 450ms	

文件名格式：MAIN_THREAD_JANK_秒级时间_进程PID.txt。
...
- 关闭开发者选项：应用在一个生命周期内，一天最多触发一次主线程超时事件堆栈采集流程。

- 启用开发者选项：应用在一个生命周期内，一小时最多触发一次主线程超时事件采集堆栈采集流程。


trace采集	主线程处理时长 > 450ms	

文件名格式：MAIN_THREAD_JANK_unix时间戳_进程PID.trace。

例如：MAIN_THREAD_JANK_1762064185461_40986.trace。

	

- 触发trace采集的前提：开发者使用nolog版本，并且关闭开发者选项。
