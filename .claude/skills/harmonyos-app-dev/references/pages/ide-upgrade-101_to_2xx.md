...
安装完成之后，进入ohpm-repo 私仓工具包解压目录下的bin目录，执行如下命令：
ohpm-repo -v
...
db:
  ...
  config:
    path: ./db     
store：旧版本仅支持本地storage存储，不支持sftp，故使用新版本store.type为fs的模板，拷贝替换旧版本store.plugin_config.path路径信息和store.plugin_config.server值至新版本对应的store.config.path和 store.config.server中。
注意
