...
配置HAP/HSP模块的限定词目录，编译时会进行过滤，匹配到的限定词目录会被打包到产物中。支持六类限定词，具体请查看表19，配置的目录要符合限定词目录取值要求。
...
是否构建字节码HAR，仅在HAR模块中配置后生效。详情请参考构建字节码HAR。
...
编译字节码HAR时，是否将sourceMap文件打包到产物中。仅HAR模块支持配置，并且只对字节码HAR生效。
...
配置include字段后，构建源码HAR时，.gitignore和.ohpmignore文件不生效，详细请参考以debug模式构建。
...
配置exclude字段后，构建源码HAR时，.gitignore和.ohpmignore文件不生效，详细请参考以debug模式构建。
...
指定需要定制的字节码HAR的abc文件的相对路径。配置后，编译时支持替换掉abc中的同名文件，以实现对字节码HAR的定制修改，具体使用方式请参考三方SDK定制修改部分源码。
