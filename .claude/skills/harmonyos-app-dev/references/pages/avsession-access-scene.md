...
SettingGeneralStateInformation.ets
进度条
...
HandlingUnsupportedCommands.ets
快进快退
...
AdaptingToBluetoothMethodOne.ets

方式二：

通过AVSession注册HandleMediaKeyEvent指令。该回调接口会直接转发媒体按键事件KeyEvent。应用需要自行识别按键事件的类型，并响应事件实现对应的功能。目前支持转发的按键事件类型如下：
...
AdaptingToBluetoothMethodTwo.ets
说明
