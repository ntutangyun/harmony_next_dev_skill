...
调用OH_CryptoEccSignatureSpec_Create传入签名数据，创建OH_CryptoEccSignatureSpec对象，用于获取转换后的数据。

调用OH_CryptoEccSignatureSpec_GetRAndS拿到转换后的数据r、s。

调用OH_CryptoEccSignatureSpec_Destroy释放内存。
